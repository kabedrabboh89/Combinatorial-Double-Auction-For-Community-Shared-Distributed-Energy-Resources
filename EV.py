...
for t in ts_all:
    ...
    revenue = 0
    for ts in range(t, t + scheduling_horizon):
        revenue += (eta_inv*PV_gen[ts]-cp.sum(charging_RE, axis=0)[ts-t]) * price_RE * grid_price[ts] + \
                   cp.sum(discharging, axis=0)[ts-t] * price_EV * grid_price[ts]
    ...
    def soc_actual(i, t_s, t_f):
        ...
        SOC = SOC_init
        for tau in range(t_s, t_f):
            SOC += (charging_grid_actual[i][tau] * eta_c[i] + charging_RE_actual[i][tau] * eta_c[i] -
                    discharging_actual[i][tau] / eta_d[i]) / capacity[i]
        return SOC

    results = pd.read_csv('./data/EV_results.csv')
...
EV_price = price_EV * grid_price
for i in range(24):
    RE_cap[i] = PV_gen[i] * eta_inv - charging_RE_total[i]
    RE_emf[i] = re_emf
    if RE_cap[i] < 0.001:
        RE_price[i] = 10
    if discharging_total[i] < 0.001:
        EV_price[i]= 10


df['RE_cap'] = RE_cap
df['RE_price'] = RE_price
df['RE_emf'] = RE_emf
df['EV_cap'] = discharging_total
df['EV_price'] = EV_price
df['EV_emf'] = grid_emf
